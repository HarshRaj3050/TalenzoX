...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const frag = /* glsl */ `
    ...
    vec3 adjustHue(vec3 color, float hueDeg) {
      // Keep the orb locked to a blue/cyan palette for the app UI.
      // Rotating the hue in this shader can push the colors toward red tones.
      return color;
    }
    ...
    const vec3 baseColor1 = vec3(0.050980, 0.321569, 0.941176);
    const vec3 baseColor2 = vec3(0.109804, 0.627451, 0.972549);
    const vec3 baseColor3 = vec3(0.011765, 0.031373, 0.145098);
    ...
    vec4 draw(vec2 uv) {
      ...
      col = mix(col, color3, smoothstep(0.9, 1.1, len) * 0.18);
  ...
  const analyzeAudio = () => {
    ...
    const frequencyData = dataArray as Uint8Array<ArrayBuffer>;
  ...
  useEffect(() => {
    ...
    try {
      ...
      glContext = rendererInstance.gl as WebGLRenderingContext | WebGL2RenderingContext;
      if (!glContext) return;
      ...
      const update = (t: number) => {
        ...
        const hasReactiveLevel = typeof reactiveLevel === "number";

        if (hasReactiveLevel) {
          voiceLevel = Math.min(Math.max(reactiveLevel, 0), 1);
